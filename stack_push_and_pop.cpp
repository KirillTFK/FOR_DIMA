...
{
    MYASSERT (stk, stk);
    if (stk->capacity < MIN_CAPACITY_TO_MULT && 3*MIN_CAPACITY_DELTA < 2*(stk->capacity - stk->size) && stk->capacity != MIN_CAPACITY )
    {
        decrease_capacity (stk);
        realloc_data (stk);
    }

    else if (stk->capacity <= MAX_CAPACITY_DELTA && 2*stk->capacity < 3*(stk->capacity - stk->size) && stk->capacity != MIN_CAPACITY)
    {
        decrease_capacity (stk);
        realloc_data (stk);
    }

    else if ( 10*MAX_CAPACITY_DELTA < 9*(stk->capacity - stk->size) && stk->capacity != MIN_CAPACITY)
    {
        decrease_capacity (stk);
        realloc_data (stk);
    }
